...
function executeAndOrStep() {
    if (!andOrState) {
        ...
        return {
            changed: false,
            rule: 'rule3',
            message: 'І-АБО граф: ініціалізація'
        };
    }
    ...
    if (andOrState.phase === 'rows') {
        if (andOrState.currentRow < gridSize.height) {
            ...
            return {
                changed: false,
                rule: 'rule3',
                message: `І-АБО: Рядок ${row + 1} без змін`
            };
        } else {
            ...
            return {
                changed: false,
                rule: 'rule3',
                message: 'І-АБО: Перехід до колонок'
            };
        }
    }
    ...
    if (andOrState.phase === 'cols') {
        if (andOrState.currentCol < gridSize.width) {
            ...
            return {
                changed: false,
                rule: 'rule3',
                message: `І-АБО: Колонка ${col + 1} без змін`
            };
        } else {
            // Всі колонки оброблено без змін
            if (!andOrState.rowsChanged) {
                ...
                // Були зміни в рядках - повторюємо цикл
                andOrState.phase = 'rows';
                andOrState.currentRow = 0;
                andOrState.rowsChanged = false;
                ...
                return {
                    changed: false,
                    rule: 'rule3',
                    message: 'І-АБО: Новий цикл обробки'
                };
            }
        }
    }
